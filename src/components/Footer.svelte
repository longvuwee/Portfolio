<footer style="background-color: #82aeca;" class="py-10 sm:py-20 border-1 border-solid border-primary flex flex-col gap-4 sm:gap-8 justify-center items-center">
    <p class="px-4 py-2 bg-white text-secondary font-medium">
        Connect with me &darr;
    </p>
    <div class="flex flex-col gap-4 items-center justify-center">
        <p><b class="pr-2">Email</b> longvu1274@gmail.com</p>
        <p><b class="pr-2">Linkedin</b>
            <a href="https://www.linkedin.com/in/long-vu-44072a23a/" target="_blank" class="text-primary hover:cursor-pointer hover:underline">Long Vu<sup>
                <span class="text-sx scale-75 pl-0.5"><i class="fa-solid fa-arrow-up-right-from-square text- xs scale-[75%]"/>
                </span>
            </sup></a>
        </p>
        <p><b class="pr-2">GitHub</b>
            <a href="https://github.com/longvuwee" target="_blank" class="text-primary hover:cursor-pointer hover:underline">longvuwee<sup>
                <span class="text-sx scale-75 pl-0.5"><i class="fa-solid fa-arrow-up-right-from-square text- xs scale-[75%]"/>
                </span>
            </sup></a>
        </p>
        <p><b class="pr-2">Resume</b>
            <a href="https://github.com/longvuwee" target="_blank" class="text-primary hover:cursor-pointer">longvuwee<sup>
                <span class="text-sx scale-75 pl-0.5"><i class="fa-solid fa-arrow-up-right-from-square text- xs scale-[75%]"/>
                </span>
            </sup></a>
        </p>
    </div>
</footer>